...
async function getStatusPageByDomain(
	domain: string,
): Promise<PublicStatusPage | null> {
	const res = await fetch(`${API_URL}/status/domain/${domain}`, {
		next: { revalidate: 30 },
	})
...
export async function generateMetadata({
	...
	const statusPage = await getStatusPageByDomain(domain)
...
export default async function StatusPageByDomain({ params }: PageProps) {
	const { host } = await params
	const domain = decodeURIComponent(host)
	const statusPage = await getStatusPageByDomain(domain)

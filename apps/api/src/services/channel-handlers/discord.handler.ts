...
async function send(ctx: AlertContext): Promise<SendResult> {
	...
	const color = isRecoveryEvent(ctx.event) ? 0x00ff00 : 0xff0000
	...
	if (ctx.cronJob) {
		...
		if (ctx.richContext?.errorPattern?.lastErrorSnippet) {
			const snippet = ctx.richContext.errorPattern.lastErrorSnippet.slice(
				0,
				100,
			)
	...
	} else if (ctx.httpMonitor) {
		fields.push({ name: "URL", value: ctx.httpMonitor.url, inline: false })
		if (ctx.httpMonitor.lastResponseMs !== null) {
			fields.push({
				name: "Response Time",
				value: `${ctx.httpMonitor.lastResponseMs}ms`,
				inline: true,
			})
		}
	}
